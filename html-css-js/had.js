...
for(let i=0; i<eData.length; i++){
  if(eData[i].dno==10){
  grademoney[0].money+=+eData[i].salary;
  d10++;
  }else if(eData[i].dno==20){
    grademoney[1].money+=+eData[i].salary;
    d20++;
  }else{
    grademoney[2].money+=+eData[i].salary;
    d30++;
  }

  
}
  grademoney[0].money=Math.floor(grademoney[0].money/d10);
  grademoney[1].money=Math.floor(grademoney[1].money/d20);
  grademoney[2].money=Math.floor(grademoney[2].money/d30);

return grademoney;
}
console.log("문제1 부서별 월급");
console.log(grade());
...
function checkPeople(){
  partPeole[0].person=d10;
  partPeole[1].person=d20;
  partPeole[2].person=d30;
  for(let i=0; i<eData.length; i++){
    if(eData[i].commission>0){
      commission1++;
    }
  }
  return partPeole;
}
console.log("2.부서별 전체 사원수와 커미션을 받는 사원들의 수")
console.log(checkPeople(),"커미션 받은 인원:"+commission1);
// 3.부서별 최대 급여와 최소 급여
...
function check30(){
 ...
 for(let i=0; i<eData.length; i++){
  if(eData[i].dno==30){
        check30[index].name=eData[i].ename;
        check30[index].title=eData[i].title;
        check30[index].dno=eData[i].no;
          check30[index].location=dData[2].location;
          index++;
      }
    }
    return check30;
  }
 console.log(" 4.부서번호가 30번인 사원들의 이름, 직급, 부서번호, 부서위치를 조회하시오.")
console.log(check30());
